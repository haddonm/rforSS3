...
#'
#' @param plotreport the list object output from teh SS_output function after
...
#'
#' @examples
#' # plotreport=plotreport; sex="Female";yrs=c(1984,2004,2016); upbound=365
plotselex <- function(plotreport,sex="Female",yrs=c(1984,2004,2016),upbound=0,
                      legendcex=1.1,labelcex=1.5) {
  if (sex %ni% c("Female","Male"))
    stop(cat("Argument 'sex' in plotselex must be either Female or Male \n"))
  sel <- plotreport$sizeselex[1:100,]
  plotprep(width=10,height=7)
  parset(plots=c(3,2),cex=1.0,byrow=FALSE)
  fleets <- plotreport$FleetNames
  nfleet <- length(fleets)
  picksex <- ifelse(sex == "Female",1,2)
  for (fleet in c(1,5,2,3,4)){ #  fleet=2
    ...
    plot(lens,selvals[1,],type="l",lwd=2,ylim=c(0,1.1),yaxs="i",
         xlab="Total Length (mm)",ylab=paste0(fleets[fleet],"_Selectivity"),
         panel.first=grid())
    L50s <- numeric(nyr); names(L50s) <- yrs
    L50s[1] <- getL50(selvals[1,])
    abline(v=L50s[1],lwd=1,col=1,lty=3)
    for(i in 2:nyr) {  #   i = 2
      lines(lens,selvals[i,],lwd=2,col=i)
      L50s[i] <- getL50(selvals[i,])
      abline(v=L50s[i],lwd=1,col=1,lty=3)
    }
    legend("bottomright",legend=yrs,col=c(1:nyr),lwd=3,bty="n",cex=legendcex)
    if (fleet == 5) plotnull(msg=sex,cex=labelcex)
  }
